...
To request a specific number of GPUs, use `--gpu-count`. GPU count requests are
mutually exclusive with explicit GPU device IDs, and the count must be greater
than zero.
...
Kubernetes-backed sandboxes honor `--gpu-count` by setting the `nvidia.com/gpu`
resource limit to the requested count. VM-backed sandboxes accept only
`--gpu-count 1`. Docker-backed and Podman-backed sandboxes currently reject GPU
count requests.
