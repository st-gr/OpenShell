...
limit. `--gpu-count` cannot be combined with `--gpu-device` because count-based
scheduling and driver-specific device selection are different allocation modes.

To set portable CPU and memory resource requests or limits, use the resource
flags:
...
Resource values use Kubernetes-style quantity strings. Kubernetes-backed
gateways map these fields into pod `resources.requests` and
`resources.limits`.

For driver-specific resource settings, pass opaque key-value entries with
`--resource-config KEY=VALUE`. The selected driver interprets those entries.
