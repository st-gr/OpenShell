...
To pass generic resource requests and limits through to the sandbox template,
use `--resources-json` with a JSON object:

```shell
openshell sandbox create \
  --resources-json '{"requests":{"cpu":"2"},"limits":{"cpu":"16"}}'
```

The JSON is forwarded as `SandboxTemplate.resources`. Use Kubernetes resource
quantity strings for CPU, memory, and extended resources.
