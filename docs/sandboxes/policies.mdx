...
When GPU devices are present in the sandbox container, OpenShell also adds the GPU filesystem baseline. This includes the injected NVIDIA or WSL2 device nodes as read-write paths. For the built-in default policy, OpenShell promotes `/proc` from read-only to read-write because CUDA initialization writes `/proc/<pid>/task/<tid>/comm`. If a custom policy explicitly keeps a GPU-required path such as `/proc` in `read_only`, sandbox startup fails and tells you to move that path to `read_write`.
