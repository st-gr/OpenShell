...
`--clear-passthrough` is mutually exclusive with `--passthrough` because passing both would be ambiguous (the server cannot honour "clear and then set" in one request — issue two updates instead). After the update the agent process receives canonical placeholders for every credential again, and any new sandbox launched against the provider picks up the revoked status on the next environment refresh.
