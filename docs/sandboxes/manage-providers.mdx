...
provider's current credential values. The gateway can mint OAuth2 client
credentials tokens and Google service account JWT tokens, then write the current
access token back to the provider record for sandbox injection.
