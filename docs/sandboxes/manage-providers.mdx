...
`--passthrough` is repeatable, and the same flag is available on `openshell provider update`. Each key must exist in the resolved provider credentials — supplied via `--credential`, discovered through `--from-existing`, or, on `update`, already stored against the provider.
