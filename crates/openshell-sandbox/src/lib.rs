...
impl BaselineEnrichmentPaths {
    ...
    fn is_gpu_read_write(&self, path: &str) -> bool {
        self.gpu_read_write.iter().any(|p| p == path)
    }
}
...
fn collect_baseline_enrichment_paths(
    ...
    let mut gpu_read_write = Vec::new();
    ...
    if include_gpu {
        ...
        for &path in GPU_BASELINE_READ_WRITE {
            push_unique(&mut read_write, path.to_string());
            push_unique(&mut gpu_read_write, path.to_string());
        }
        for path in gpu_device_nodes {
            push_unique(&mut read_write, path.clone());
            push_unique(&mut gpu_read_write, path);
        }
    }
...
fn proto_policy_source(proto: &openshell_core::proto::SandboxPolicy) -> BaselinePolicySource {
    if proto == &openshell_policy::restrictive_default_policy() {
        BaselinePolicySource::DefaultLike
    } else {
        BaselinePolicySource::Custom
    }
}

fn gpu_read_write_conflict(path: &str) -> miette::Report {
    miette::miette!(
    ...
    F: Fn(&str) -> bool,
{
    ...
    for path in &paths.read_write {
        ...
        if fs.read_write.iter().any(|p| p == path) {
            if source == BaselinePolicySource::DefaultLike && paths.is_gpu_read_write(path) {
        ...
        if fs.read_only.iter().any(|p| p == path) {
            if paths.is_gpu_read_write(path) {
                if source == BaselinePolicySource::Custom {
                    return Err(gpu_read_write_conflict(path));
                }
                fs.read_only.retain(|p| p != path);
...
fn enrich_sandbox_baseline_paths(policy: &mut SandboxPolicy) -> Result<bool> {
    ...
    let paths = active_baseline_enrichment_paths(include_proxy);
    enrich_sandbox_baseline_paths_with(policy, &paths, BaselinePolicySource::Custom, |path| {
        std::path::Path::new(path).exists()
    })
}
...
async fn load_policy(
    ...
    if let (Some(id), Some(endpoint)) = (&sandbox_id, &openshell_endpoint) {
        ...
        let mut proto_policy = if let Some(p) = proto_policy {
            ...
            enrich_proto_baseline_paths_from_source(
                &mut discovered,
                BaselinePolicySource::DefaultLike,
            )?;
